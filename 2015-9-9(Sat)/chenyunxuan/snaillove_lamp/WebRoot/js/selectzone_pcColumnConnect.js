...
//            搜索框的id属性名称
            ...
            var yiwei=new Array(); 
             ...
             var cloud_id=yiwei[0];
             var name=yiwei[1];
             var b_id=yiwei[2];
             var type_=yiwei[3]
           
            
            var url ="/music/web/addcloud.action";
            var result = Object();
            $.post(url, {"name": name, "cloud_id": cloud_id,"b_id": b_id,"type_":type_},
            function(data) {
                 try {
                  addElementobj(_self.targetSel, data);
                  alert("关联成功！");
    ...
    this.dropItem = function(all, act)
    {
        ...
        if (arr.length > 0)
        {
            ...
            var yiwei=new Array(); 
             yiwei=ids.split(",");
              var b_id=yiwei[0];
             var cloud_id=yiwei[1];
             var type=yiwei[2];
            
           
            var url = "/music/web/delcloud.action";
            $.post(
            	url, {"b_id": b_id,"type":type},
            function(data) {
            	if(data.status == 1){
            		addElement(_self.targetSel, null);
               alert("移除成功！");
