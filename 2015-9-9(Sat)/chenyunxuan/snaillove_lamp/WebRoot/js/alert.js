...
var Pop = function(options) {
	...
	// 是否添加浮动层收缩
	if (!!this.options.idShrink)
		this.Shrink(this.options.idShrink);

	/* 关闭、打开浮动层 */
	if (this.options.idClose.length != 0)
		this.Close(this.options.idClose);
...
Pop.prototype = {
	...
	GetValue : function(mark) {
		if (this.oPop.css("display") == "none")
			return; // 浮动层显示时才动态计算其坐标(解决一个效率问题)
		var level = this.mode[0].toLowerCase(), vertical = this.mode[1]
				.toLowerCase();
		this.Initialization();
		if (mark) {
			if (!this.isIE6) {
				...
				var x_final = level == "left" ? this.leftScroll + this.left
						: this.widthWindow + this.leftScroll - this.widthPop
								- this.right;
				var y_final = vertical == "top" ? this.topScroll + this.top
						: this.heightWindow + this.topScroll - this.heightPop
								- this.bottom;
				this.oPop.css({
					top : y_final,
					left : x_final
			...
			if (!this.isIE6) {
				...
				var x_final = (this.widthWindow - this.widthPop) / 2
						+ this.leftScroll;
				var y_final = (this.heightWindow - this.heightPop) / 2
						+ this.topScroll;
			}
			this.oPop.css({
				top : y_final,
	...
	CenterLocation : function() {
		...
		$(window).bind("scroll." + this.options.oPop, function() {
			...
			_this.GetValue(false);
			if (_this.oPop.css("display") != "none")
