...
//            alert(result.message);
     ...
     */
    this.addItem5 = function(all, act)
    {
...
//            ${special.id},${special.name}
            var yiwei=new Array(); 
             ...
             var id=yiwei[0];//提供商标签ID
             var name=yiwei[1];     //提供商标签名称
             var label_id = $("#label_id").val();//自定义的 id
            
           
//            alert("------->code"+code+"----id"+zidingyi_id);addSpecial.action

            var url ="/music/web/addSpecialLabel.action";
            var result = Object();
            $.post(url, {"id": id, "name": name,"label_id": label_id},
            function(data) {
                 try {
...
//                  alert("关联成功！");
     ...
     * @param   string     act
     */
    this.dropItem5 = function(all, act)
    {
        if (!this.check())
        {
            return;
...
//             alert(ids);
            ...
            var url = "/music/web/delSpecialLabel.action";
            $.post(
            	url, {"id": ids,"label_id":label_id},
            function(data) {
            		  try {
...
function addElement(obj, list)
{
    ...
    if (list != null && list != "" && list != undefined)
    {
        ...
        {
            var opt = document.createElement("OPTION");
//            opt.value = list[i].id+","+list[i].p_id;
            opt.value = list[i].id;
